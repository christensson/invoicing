...
//if password matches take into website
//if user doesn't exist or password doesn't match tell them it failed
exports.findOrCreate = function(idField, userData, license) {
  var deferred = Q.defer();
  ...
  mydb.getUser(query).then(function (existingUser) {
    ...
    if (err.message === 'The requested items could not be found.') {
      ...
      mydb.addUser(userData).then(function() {
        ...
        mydb.initUserContext(query, license).then(function(newUser) {
          var result = {
              "user": newUser,
              "isNew": true,
          };
...
exports.createUserInfo = function(name, email) {
  var info = {
      "name": name,
      "email": email,
